...
async function createAuditLog(
  ...
  console.log(
    "📝 创建审计日志:",
    `${username} - ${action} - ${module} - ${details} [位置: ${locationInfo}]`
  );
  return log;
}
...
const ROUTE_PERMISSIONS_FILE = path.join(
  process.cwd(),
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    if (user && user.password === password) {
      ...
      const token = `token_${user.id}_${Date.now()}_${Math.random()
        .toString(36)
        .substring(2)}`;
...
app.post("/api/register", (req, res) => {
  try {
    ...
    // 检查用户名是否已存在
    const existingUserByUsername = Object.values(users).find(
    ...
    // 为新用户分配默认路由权限
    routePermissions.userRoutePermissions[newUser.username] = [
    ...
    console.log(
      `   路由权限: ${routePermissions.userRoutePermissions[
        newUser.username
      ].join(", ")}`
    );
    ...
    Object.values(users).forEach((user, index) => {
      console.log(
        `   ${index + 1}. ${user.username} (ID: ${user.id}, 邮箱: ${
          user.email
...
app.post("/api/users", (req, res) => {
  try {
    ...
    if (Array.isArray(routePermissions) && routePermissions.length > 0) {
      ...
      // 保存到路由权限配置中
      routePermissionsConfig.userRoutePermissions[newUser.username] =
        routePermissions;
      ...
      routePermissionsConfig.userRoutePermissions[newUser.username] = [
        ...routePermissionsConfig.defaultPermissions,
      ];
    }
    ...
    fs.writeFileSync(
      "./routePermissions.json",
      JSON.stringify(routePermissionsConfig, null, 2)
    );
    ...
    console.log(
      `   路由权限: ${routePermissionsConfig.userRoutePermissions[
        newUser.username
      ].join(", ")}`
    );
...
app.get("/api/user/:userId/route-permissions", (req, res) => {
  try {
    ...
    const userPermissions =
      routePermissionsConfig.userRoutePermissions[targetUser.username] ||
      routePermissionsConfig.defaultPermissions;

    // 获取所有可用权限
    const allAvailablePermissions =
      routePermissionsConfig.availableRoutes || [];

    // 按照用户权限配置的顺序生成权限详情（保持原有顺序）
    const userPermissionDetails = userPermissions
      .map((permissionName) => {
        return allAvailablePermissions.find(
          (route) => route.name === permissionName
        );
      })
...
app.put("/api/user/:userId/route-permissions", (req, res) => {
  try {
    ...
    // 检查所有权限是否在可用列表中
    const availableRouteNames = routePermissionsConfig.availableRoutes.map(
      (route) => route.name
    );
    const invalidPermissions = permissions.filter(
      (perm) => !availableRouteNames.includes(perm)
    );
...
app.get("/api/user/:userId/routes", (req, res) => {
  try {
    ...
    if (routePermissions.userRoutePermissions[targetUser.username]) {
      userRouteConfig =
        routePermissions.userRoutePermissions[targetUser.username];
    } else {
      // 如果没有对应的配置，根据用户权限选择默认配置
      if (targetUser.permissions.includes("admin")) {
        userRouteConfig =
          routePermissions.userRoutePermissions.admin ||
          routePermissions.defaultPermissions;
      } else {
        userRouteConfig =
          routePermissions.userRoutePermissions.user ||
          routePermissions.defaultPermissions;
      }
    }

    // 生成路由配置
    const routes = generateUserRoutesFromPermissions(
      userRouteConfig,
      routePermissions,
  ...
  try {
    ...
    function generateMockData(count) {
      ...
      return Array.from({ length: count }, (_, i) => ({
        ...
        createTime: `2023-${String(Math.floor(Math.random() * 12) + 1).padStart(
          2,
          "0"
...
// 根据权限列表生成路由配置
function generateUserRoutesFromPermissions(
  allowedRoutes,
  routePermissions,
  userPermissions
) {
  ...
  allowedRoutes.forEach((routeName) => {
    ...
    if (route) {
      ...
      if (route.parent) {
        ...
        if (!parentRoute) {
          // 如果父路由还没有添加，先添加父路由
          const parentRouteConfig = availableRoutes.find(
            (r) => r.name === route.parent
          );
          if (
            parentRouteConfig &&
            allowedRoutes.includes(parentRouteConfig.name)
          ) {
            parentRoute = {
        ...
        if (parentRoute) {
          ...
          // 处理子路由的相对路径
          const childPath =
            route.path.replace(parentRoute.path, "").replace(/^\//, "") ||
...
function generateUserRoutes(userPermissions, routePermissions) {
  const { availableRoutes, userRoutePermissions, defaultPermissions } =
    routePermissions;
  ...
  try {
    ...
    if (!targetUser.profile) {
      targetUser.profile = {
        ...
        twoFactorEnabled: false,
        lastPasswordChange:
          targetUser.registeredAt || new Date().toISOString().split("T")[0],
      };
      // 保存更新
      saveUsers();
    }
...
app.put("/api/profile/:userId", (req, res) => {
  try {
    ...
    if (twoFactorEnabled !== undefined)
      targetUser.profile.twoFactorEnabled = twoFactorEnabled;
...
app.post("/api/profile/:userId/avatar", upload.single("avatar"), (req, res) => {
  try {
    ...
    if (
      ...
      targetUser.profile.avatarUrl.startsWith("/uploads/")
    ) {
...
app.put("/api/profile/:userId/password", (req, res) => {
  try {
    ...
    if (!targetUser.profile) {
      ...
      targetUser.profile.lastPasswordChange = new Date()
        .toISOString()
        .split("T")[0];
    }
...
app.get("/api/audit-logs", (req, res) => {
  try {
    ...
    console.log(
      `📄 获取审计日志，筛选后数量: ${total}，返回: ${paginatedLogs.length}`
    );
...
app.get("/api/audit-logs/stats/:userId", (req, res) => {
  try {
    ...
    const todayLogs = targetLogs.filter((log) =>
    ...
    const stats = {
      ...
      recentActions: Object.entries(actionStats).map(([action, count]) => ({
        action,
        count,
      })),
      moduleStats: Object.entries(moduleStats).map(([module, count]) => ({
        module,
        count,
      })),
    };
...
app.delete("/api/audit-logs/:logId", (req, res) => {
  try {
    ...
    console.log(
      `🗑️ 删除审计日志: ${deletedLog.username} - ${deletedLog.action} - ${deletedLog.module}`
    );
