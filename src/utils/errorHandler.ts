...
class ErrorHandler {
    ...
    private setupGlobalHandlers() {
        ...
        // 捕获Vue错误（如果使用Vue）
        if (window.Vue || (window as any).app) {
    ...
    // 处理错误
    public handleError(errorInfo: ErrorInfo, level: ErrorLevel = ErrorLevel.ERROR) {
        // 添加额外信息
        const enrichedError = {
            ...errorInfo,
            level,
    ...
    private getConsoleStyle(level: ErrorLevel): string {
        const styles = {
            ...
            [ErrorLevel.FATAL]: 'color: #FFFFFF; background: #F44336; font-weight: bold; padding: 2px 6px; border-radius: 3px;'
        };
        return styles[level];
    }
    ...
    // 手动报告错误
    public reportManualError(message: string, extra?: any, level: ErrorLevel = ErrorLevel.ERROR) {
        this.handleError({
            message,
            timestamp: new Date().toISOString(),
            url: window.location.href,
...
// 导出便捷方法
export function reportError(message: string, extra?: any, level: ErrorLevel = ErrorLevel.ERROR) {
