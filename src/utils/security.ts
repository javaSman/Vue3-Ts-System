...
// SQL注入防护 - 基本检查
export function detectSqlInjection(input: string): { safe: boolean; message: string } {
    const sqlPatterns = [
        /('|(\\'))+.*(OR|or).*(('|(\\')))+.*((=)|(==)|[>]|[<])/,
...
// 表单数据验证
export function validateFormData(data: Record<string, any>, rules: Record<string, any>): {
    valid: boolean;
