...
export async function fetchUserRoutePermissions(userId: number): Promise<{
    ...
    try {
        const response = await axiosInstance.get(`/user/${userId}/route-permissions`);
        return response.data;
    } catch (error: any) {
        if (error.response && error.response.data) {
            return {
...
// 更新用户的路由权限
export async function updateUserRoutePermissions(userId: number, permissions: string[]): Promise<{
    success: boolean;
    message: string;
    data?: any;
}> {
    try {
        const response = await axiosInstance.put(`/user/${userId}/route-permissions`, {
            permissions
        });
        return response.data;
    } catch (error: any) {
