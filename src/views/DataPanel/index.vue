  ...
  <div class="dashboard-container">
    ...
    <div class="header">
      ...
      <div class="header-controls">
        ...
        <button @click="toggleAutoUpdate" :class="['refresh-btn', autoUpdate ? 'active' : '']">
    ...
    <div class="overview-stats">
      <div class="stat-card online">
        ...
        <div class="content">
          <h3>在线设备</h3>
          <div class="value">{{ overviewData.onlineDevices }}/{{ overviewData.totalDevices }}</div>
          <div class="percentage">{{ Math.round(overviewData.onlineDevices / overviewData.totalDevices * 100) }}%</div>
        </div>
      </div>
      ...
      <div class="stat-card data">
        ...
        <div class="content">
          <h3>今日数据量</h3>
          <div class="value">{{ formatNumber(overviewData.todayDataPoints) }}</div>
    ...
    <div class="data-section">
      ...
      <div class="table-container">
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="device in deviceData" :key="device.id" :class="{ 'warning-row': device.status === 'warning' }">
              <td>{{ device.id }}</td>
              <td>{{ device.name }}</td>
              <td>
                <span class="status-badge" :class="device.status">{{ getStatusText(device.status) }}</span>
              </td>
              <td :class="getValueClass(device.temperature, 70, 85)">{{ device.temperature }}</td>
              <td :class="getValueClass(device.humidity, 80, 90)">{{ device.humidity }}</td>
              <td :class="getValueClass(device.voltage, 210, 250, true)">{{ device.voltage }}</td>
              <td>{{ device.uptime }}</td>
  ...
  if (statusChartInstance && statusChart.value) {
    ...
    const option = {
      ...
      series: [{
        ...
        radius: ['40%', '70%'],
        data: [
          { value: statusCount.normal, name: '正常', itemStyle: { color: '#52c41a' } },
          { value: statusCount.warning, name: '警告', itemStyle: { color: '#faad14' } },
          { value: statusCount.offline, name: '离线', itemStyle: { color: '#d9d9d9' } },
        ],
      }],
    }
...
// ======================
const initCharts = () => {
  if (temperatureChart.value) temperatureChartInstance = echarts.init(temperatureChart.value)
...
const getValueClass = (value: number, warnThresh: number, dangerThresh: number, reverse = false): string => {
  if (reverse) {
